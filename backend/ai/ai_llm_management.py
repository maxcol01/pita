...
def format_ingredients_for_prompt(items):
    lines = []
    for item in items:
        lines.append(f"- {item['name']} ({item['quantity']} {item['unit']}) from {item["location"]}")
    return "\n".join(lines)


def generate_response(formated_ingredients):
    # Prompt
    prompt = f"""
            ...
            ### Rules:
            1. Use as many pantry ingredients as possible BUT of course the recipe must be consistent (DO NOT MIX NEEDLESSLY ingredients for the sake of using everything. BE SMART ABOUT IT).
